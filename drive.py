...
def list_folder_files_with_ids(folder_id: str) -> list[dict]:
    """Return list of {name, id, webViewLink} for all files in a Drive folder (recursive)."""
    service = get_service()

    def _list_recursive(fid: str) -> list[dict]:
        files = []
        page_token = None
        while True:
            resp = (
            ...
            for item in resp.get("files", []):
                if item["mimeType"] == "application/vnd.google-apps.folder":
                    ...
                    files.extend(_list_recursive(item["id"]))
                else:
                    files.append(item)
            page_token = resp.get("nextPageToken")
            if not page_token:
    ...
    return _list_recursive(folder_id)
